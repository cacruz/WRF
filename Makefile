...
explis :
	@ echo '----------- WRF/LIS ----------------------'
	$(MAKE) -C $(NUWRFDIR)/LISF/lis/make explis

# NUWRF EMK
# LIS as standalone executable
# Note: MacOSX needs the .PHONY attribute to work.
.PHONY: LIS
LIS:
	@ echo '----------- LIS ----------------------'
	$(MAKE) -C $(NUWRFDIR)/LISF/lis/make LIS
...
em_core :
	...
	if [ $(WRF_CHEM) -eq 0 ] ; then \
		...
		( cd dyn_em ; $(MAKE) $(J) CF="$(CHEM_FILES)" ) ; \
	fi
